...
def main():
    ap=argparse.ArgumentParser(description="C27v2 exact-source boundary-state reconstruction.")
    ap.add_argument("--month",type=int,required=True)
    ap.add_argument("--data-dir",default=os.environ.get("R9_DATA_DIR","/mnt/data"))
    ap.add_argument("--out-dir",default=os.environ.get("R9_C27V2_OUT_DIR","/mnt/data/C27v2_boundary_state"))
    a=ap.parse_args()
    m=a.month
    out=Path(a.out_dir)
    ...
    raw=candidates[0]
    job=f"C27V2_BOUNDARY_STATE_{m:02d}"
    manifest=out/f"{job}.json"
    started=time.time()
    atomic_json(manifest,{"job_id":job,"status":"STARTED","month":m,"source":Path(raw).name})

    t,ask,bid,mid=load_ticks(raw)

    secs=t//1000
    uniq_sec,sec_hi,sec_lo,sec_row=grouped_bars(secs,bid)
    sec_bhi,sec_blo=previous_window_extremes(sec_hi,sec_lo,60)

    mins=t//60000
    uniq_min,min_hi,min_lo,min_row=grouped_bars(mins,bid)
    ...
    for sc in SCALES:
        ...
        for side in (-1,1):
            for st in ("ACCEPT","RECLAIM"):
                q=z[(z.event_side==side)&(z.event_type==st)]
                for act,col in (("CONTINUE","cont_pnl"),("FADE","fade_pnl")):
                    met=metrics(q[col].to_numpy())
                    summary.append({"month":m,"scale":sc,"event_side":int(side),"event_type":st,
                                    "action":act,"boundary_kind":boundary_kind,**met})
        del et,es,typ,bp,cp,ce,ch,fp,fe,fh

    events=pd.concat(frames,ignore_index=True).sort_values(["event_ms","scale"]).reset_index(drop=True)
    ...
    final={
        "job_id":job,"status":"COMPLETED_LOCAL","month":m,
        "ticks":int(len(t)),"events":int(len(events)),"state_rows":int(len(sm)),
        ...
        "event_detection":{
            ...
            "scale1":"rolling 60 most recent completed tick-containing 1s BID OHLC bars; current forming second excluded",
            "scales_3_5_10_20":"previous N completed tick-containing M1 BID OHLC bars",
