...
#property strict
#property description "Gold MUWHAHA R9 GAMMA-01 candidate: session-adaptive dual-horizon temporal expansion gate plus validated lifecycle."
...
input int    InpMinRangePips           = 400; // GAMMA-01: 10-active-second range >= 4.00
input int    InpMaxTurns               = 9;

input group "GAMMA-01 dual-horizon temporal expansion"
input bool   InpUseContextRangeGate       = true;
input int    InpContextRangeLookbackSec   = 45;  // active completed seconds, same causal ring semantics as R9 S1
input int    InpContextRangeOffPips       = 800; // 8.00
input int    InpContextRangeLondonPips    = 800; // 8.00
input int    InpContextRangeOverlapPips   = 1400;// 14.00
input int    InpContextRangeNewYorkPips   = 1400;// 14.00
...
int ContextRangeMinimumPips(const ENUM_GMM_SESSION s)
{
   if(s==GMM_SESSION_OVERLAP) return InpContextRangeOverlapPips;
   if(s==GMM_SESSION_NEWYORK) return InpContextRangeNewYorkPips;
   if(s==GMM_SESSION_LONDON) return InpContextRangeLondonPips;
   return InpContextRangeOffPips;
}
...
bool S1QualityForSide(const int side,double &disp,double &eff,double &range10,double &range45,int &turns)
{
   disp=0.0;eff=0.0;range10=0.0;range45=0.0;turns=0;
   ...
   if(InpUseContextRangeGate)
   {
      if(InpContextRangeLookbackSec<2 || InpContextRangeLookbackSec>=MICRO_CAPACITY || g_microCount<InpContextRangeLookbackSec)
         return false;
      double hi45=-DBL_MAX,lo45=DBL_MAX;
      for(int off=0;off<InpContextRangeLookbackSec;++off)
      {
         const int idx=RingIndexFromNewest(off);
         if(g_micro[idx].high>hi45) hi45=g_micro[idx].high;
         if(g_micro[idx].low<lo45) lo45=g_micro[idx].low;
      }
      range45=hi45-lo45;
      const int minContextPips=ContextRangeMinimumPips(CurrentSession());
      if(minContextPips>0 && range45+1e-12<HunterDistance(minContextPips)) return false;
   }
...
void ProcessEntries(const MqlTick &tick)
{
   ...
   double disp,eff,rng,rng45;int turns;
   if(!S1QualityForSide(side,disp,eff,rng,rng45,turns)) return;
   if(InpVerbose) PrintFormat("%s: qualified side=%d disp=%.3f eff=%.3f range10=%.3f range45=%.3f turns=%d",EA_TAG,side,disp,eff,rng,rng45,turns);
   OpenTrade(side,tick);
}
...
int OnInit()
{
   if(InpLots<=0.0 || InpGapPips<=0 || InpStopLossPips<=0 || InpTrailPips<0 || InpTrailActivationPips<0 ||
      ...
      InpContextRangeLookbackSec<2 || InpContextRangeLookbackSec>=MICRO_CAPACITY ||
      InpContextRangeOffPips<0 || InpContextRangeLondonPips<0 || InpContextRangeOverlapPips<0 || InpContextRangeNewYorkPips<0 ||
   ...
   PrintFormat("%s initialized gap=%d stop=%d trail=%d act=%d velLB=%d minVel=%d eff=%.2f range10=%d contextLB=%d ctxOff=%d ctxLon=%d ctxOvl=%d ctxNY=%d maxHold=%d rearms=%d",
               EA_TAG,InpGapPips,InpStopLossPips,InpTrailPips,InpTrailActivationPips,InpVelocityLookbackSec,InpMinVelocityPips,InpMinDirectionalEff,InpMinRangePips,
               InpContextRangeLookbackSec,InpContextRangeOffPips,InpContextRangeLondonPips,InpContextRangeOverlapPips,InpContextRangeNewYorkPips,
               InpMaxHoldSeconds,InpMaxSameMinuteRearms);
...
void OnTick()
{
   MqlTick tick;if(!SymbolInfoTick(_Symbol,tick) || tick.bid<=0.0 || tick.ask<=0.0) return;Reconcile(tick);
}
