...
#property strict
#property description "Gold MUWHAHA R9 GAMMA-01: exact R9 plus session-normalized M5 ATR relative-volatility gate."
...
input double InpMinSessionAtrRatio      = 1.50; // GAMMA-01: completed M5 ATR / R9 session ATR floor
...
string EA_TAG = "GM_R9_GAMMA_01_REL_VOL";
...
bool GateAllowsEntry(const MqlTick &tick)
{
   ...
   if(mn>0.0 && (atr+1e-12<mn || atrRatio+1e-12<InpMinSessionAtrRatio))
   {
      const datetime bar=iTime(_Symbol,PERIOD_M1,0);
      if(InpVerbose && bar!=g_lastGateLogBar)
      {
         PrintFormat("%s: gate CLOSED ATR=%.5f min=%.5f ratio=%.3f need=%.3f spread=%.1f",
                     EA_TAG,atr,mn,atrRatio,InpMinSessionAtrRatio,sp);
...
int OnInit()
{
   if(InpLots<=0.0 || InpGapPips<=0 || InpStopLossPips<=0 || InpTrailPips<0 || InpTrailActivationPips<0 ||
      ...
      InpHunterPipPrice<=0.0 || InpAtrPeriod<=0 || InpMaxSpreadPoints<0 || InpMinSessionAtrRatio<1.0)
   {
   ...
   PrintFormat("%s initialized gap=%d stop=%d trail=%d act=%d velLB=%d minVel=%d eff=%.2f range=%d maxHold=%d rearms=%d minSessionAtrRatio=%.2f",
               EA_TAG,InpGapPips,InpStopLossPips,InpTrailPips,InpTrailActivationPips,InpVelocityLookbackSec,InpMinVelocityPips,InpMinDirectionalEff,InpMinRangePips,InpMaxHoldSeconds,InpMaxSameMinuteRearms,InpMinSessionAtrRatio);
