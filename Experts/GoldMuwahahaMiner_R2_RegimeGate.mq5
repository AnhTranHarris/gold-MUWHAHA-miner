...
#property strict
#property description "Gold MUWHAHA R2: R1 geometry plus completed-M5 ATR and live-spread entry gates."

#include <Trade/Trade.mqh>

CTrade trade;

input group "R2 geometry"
...
bool GateAllowsEntry()
{
   ...
   if(InpMaxSpreadPoints > 0 && spreadPoints > (double)InpMaxSpreadPoints + 1e-9)
   {
      ...
      {
         PrintFormat("%s: entry gate CLOSED: spread %.1f > %d points",
                     EA_TAG, spreadPoints, InpMaxSpreadPoints);
   ...
   if(!haveAtr)
   {
      if(InpFailClosedOnNoATR)
      {
         const datetime bar = iTime(_Symbol, PERIOD_M1, 0);
         if(InpVerbose && bar != g_lastGateLogBar)
         {
            Print(EA_TAG, ": entry gate CLOSED: completed ATR unavailable");
            g_lastGateLogBar = bar;
         }
         return false;
      }
   ...
   if(InpMinAtrPrice > 0.0 && atrValue + 1e-12 < InpMinAtrPrice)
   {
      const datetime bar = iTime(_Symbol, PERIOD_M1, 0);
      if(InpVerbose && bar != g_lastGateLogBar)
      {
         PrintFormat("%s: entry gate CLOSED: completed M5 ATR %.5f < %.5f",
                     EA_TAG, atrValue, InpMinAtrPrice);
         g_lastGateLogBar = bar;
      }
      return false;
   }
...
void Reconcile()
{
   ...
   if(currentBar != 0 && currentBar != g_cycleBar)
   {
      ...
      // Mark the minute as processed even when the regime gate rejects it.
      // This prevents repeated intraminute retries and keeps the test free of
      // look-ahead from a gate that becomes favorable later in the same M1 bar.
...
int OnInit()
{
   ...
   PrintFormat("%s initialized on %s; gap=%d trail=%d ATR(%s,%d)>=%.2f spread<=%d points",
               EA_TAG,
               _Symbol,
               InpGapPips,
               InpTrailPips,
