...
#property strict
#property description "Gold MUWHAHA R7: R5 100/30/8/8 plus adaptive drawdown governor and stressed re-arm throttle."
...
input group "R7 session-adaptive ATR"
...
input group "R7 drawdown governor"
input bool   InpUseDrawdownGovernor   = true;
input double InpGovernorAmberR        = 8.0;
input double InpGovernorRedR          = 10.0;
input double InpGovernorRecoverR      = 6.0;
input int    InpGovernorCooldownMin   = 45;
input double InpGovernorDailyFloorR   = 18.0;
input bool   InpThrottleRearmsStress  = true;
...
// Governor state is based on closed P/L for this EA on the current broker day.
datetime g_governorDayStart   = 0;
double   g_governorDayPeak    = 0.0;
bool     g_governorRedLatched = false;
datetime g_governorCooldownUntil = 0;
...
bool IsLondonDstUtc(const datetime utc)
{
   ...
   return (utc >= MakeUtc(now.year, 3, startDay, 1, 0) &&
           utc <  MakeUtc(now.year, 10, endDay, 1, 0));
}
...
bool IsNewYorkDstUtc(const datetime utc)
{
   ...
   return (utc >= MakeUtc(now.year, 3, startDay, 7, 0) &&
           utc <  MakeUtc(now.year, 11, endDay, 6, 0));
}
...
{
   if(session == GMM_SESSION_LONDON) return "LONDON";
   if(session == GMM_SESSION_OVERLAP) return "OVERLAP";
   if(session == GMM_SESSION_NEWYORK) return "NEWYORK";
   return "OFF";
}
   ...
   if(minimumAtr > 0.0 && atrValue + 1e-12 < minimumAtr)
   {
      ...
      if(InpVerbose && bar != g_lastGateLogBar)
      {
         PrintFormat("%s: gate CLOSED session=%s ATR=%.5f minimum=%.5f spread=%.1f",
                     EA_TAG, SessionName(session), atrValue, minimumAtr, spreadPoints);
...
void FindOurPending(ulong &buyTicket, ulong &sellTicket)
{
   ...
   for(int index = OrdersTotal() - 1; index >= 0; --index)
   {
      ...
      if(ticket == 0 || !IsOurPendingSelected()) continue;
      const ENUM_ORDER_TYPE type = (ENUM_ORDER_TYPE)OrderGetInteger(ORDER_TYPE);
      if(type == ORDER_TYPE_BUY_STOP && buyTicket == 0) buyTicket = ticket;
      else if(type == ORDER_TYPE_SELL_STOP && sellTicket == 0) sellTicket = ticket;
   }
}
...
void DeleteAllOurPending()
{
   ...
   if(buyTicket != 0) DeletePending(buyTicket);
   if(sellTicket != 0) DeletePending(sellTicket);
}
...
{
   const datetime dayStart = BrokerDayStart();
...
double GovernorRiskMoney()
{
   ...
   if(!OrderCalcProfit(ORDER_TYPE_BUY,
                       ...
                       volume,
                       tick.ask,
                       tick.ask - stopDistance,
                       result))
      return 0.0;
   return MathAbs(result);
}

void ResetGovernorDayIfNeeded()
{
   if(!InpUseDrawdownGovernor) return;
   const datetime dayStart = BrokerDayStart();
   if(g_governorDayStart == dayStart) return;
   g_governorDayStart = dayStart;
   const double nowProfit = ClosedProfitToday();
   g_governorDayPeak = MathMax(0.0, nowProfit);
   g_governorRedLatched = false;
   g_governorCooldownUntil = 0;
   Log("governor day reset");
}

void UpdateGovernor()
{
   if(!InpUseDrawdownGovernor) return;
   ResetGovernorDayIfNeeded();
   const double riskMoney = GovernorRiskMoney();
   if(riskMoney <= 0.0) return;
   const double nowProfit = ClosedProfitToday();
   if(nowProfit > g_governorDayPeak)
      g_governorDayPeak = nowProfit;
   const double ddR = (g_governorDayPeak - nowProfit) / riskMoney;
   if(g_governorRedLatched && ddR <= InpGovernorRecoverR)
   {
      g_governorRedLatched = false;
      Log(StringFormat("governor RECOVERED dd=%.2fR", ddR));
   }
   if(!g_governorRedLatched && ddR >= InpGovernorRedR)
   {
      g_governorRedLatched = true;
      g_governorCooldownUntil = TimeCurrent() + (datetime)(InpGovernorCooldownMin * 60);
      Log(StringFormat("governor RED dd=%.2fR cooldown=%d min", ddR, InpGovernorCooldownMin));
   }
}

bool GovernorDailyFloorHit()
{
   if(!InpUseDrawdownGovernor || InpGovernorDailyFloorR <= 0.0) return false;
   const double riskMoney = GovernorRiskMoney();
   if(riskMoney <= 0.0) return false;
   return (ClosedProfitToday() <= -(InpGovernorDailyFloorR * riskMoney));
}

double GovernorDrawdownR()
{
   if(!InpUseDrawdownGovernor) return 0.0;
   const double riskMoney = GovernorRiskMoney();
   if(riskMoney <= 0.0) return 0.0;
   const double nowProfit = ClosedProfitToday();
   return MathMax(0.0, (g_governorDayPeak - nowProfit) / riskMoney);
}

bool GovernorStressActive()
{
   if(!InpUseDrawdownGovernor) return false;
   UpdateGovernor();
   return (g_governorRedLatched || GovernorDrawdownR() >= InpGovernorAmberR);
}

bool GovernorAllowsFresh()
{
   if(!InpUseDrawdownGovernor) return true;
   UpdateGovernor();
   if(GovernorDailyFloorHit()) return false;
   if(TimeCurrent() < g_governorCooldownUntil) return false;
   // After red cooldown, probe-mode fresh brackets are allowed even while latched.
   return true;
}

bool GovernorAllowsRearm()
{
   if(!InpUseDrawdownGovernor || !InpThrottleRearmsStress) return true;
   UpdateGovernor();
   if(GovernorDailyFloorHit()) return false;
   if(TimeCurrent() < g_governorCooldownUntil) return false;
   if(GovernorStressActive()) return false;
   return true;
}
...
bool PlaceBuyStopAtCycleBoundary()
{
   if(g_cycleBuyPrice <= 0.0 || g_cycleSellPrice <= 0.0 ||
      !GovernorAllowsRearm() || !GateAllowsEntry())
      return false;
   const double volume = NormalizeVolume(InpLots);
   const double sl = NormalizePrice(g_cycleBuyPrice - HunterDistance(InpStopLossPips));
   ResetLastError();
   const bool sent = trade.BuyStop(volume, g_cycleBuyPrice, _Symbol, sl, 0.0,
...
bool PlaceSellStopAtCycleBoundary()
{
   if(g_cycleBuyPrice <= 0.0 || g_cycleSellPrice <= 0.0 ||
      ...
      return false;
   const double volume = NormalizeVolume(InpLots);
   const double sl = NormalizePrice(g_cycleSellPrice + HunterDistance(InpStopLossPips));
   ResetLastError();
   const bool sent = trade.SellStop(volume, g_cycleSellPrice, _Symbol, sl, 0.0,
...
bool PlaceFreshMinuteBracket(const datetime barTime)
{
   ...
   const double volume = NormalizeVolume(InpLots);
   if(buyPrice <= tick.ask || sellPrice >= tick.bid || volume <= 0.0) return false;
   ResetLastError();
                    ...
                    const double currentSL)
{
   ...
   {
      if((tick.bid - openPrice) < activationDistance) return;
   ...
   {
      if((openPrice - tick.ask) < activationDistance) return;
...
void Reconcile()
{
   ...
   if(havePosition)
   {
      ...
      if(positionType == POSITION_TYPE_BUY && sellTicket != 0) DeletePending(sellTicket);
      else if(positionType == POSITION_TYPE_SELL && buyTicket != 0) DeletePending(buyTicket);
      ManageTrailing(positionTicket, positionType, openPrice, currentSL);
      g_reconcileBusy = false;
      return;
   }

   if(DailyTargetReached() || GovernorDailyFloorHit())
   {
      DeleteAllOurPending();
      g_reconcileBusy = false;
      return;
   ...
   if(g_wasInPosition)
   {
      ...
      if(currentBar == g_cycleBar && g_cycleBuyPrice > 0.0 && g_cycleSellPrice > 0.0)
      {
         if(buyTicket != 0) DeletePending(buyTicket);
         if(sellTicket != 0) DeletePending(sellTicket);
         if(GovernorAllowsRearm())
         {
            if(g_lastPositionType == POSITION_TYPE_SELL) PlaceBuyStopAtCycleBoundary();
            else PlaceSellStopAtCycleBoundary();
         }
         g_reconcileBusy = false;
         return;
      }
...
int OnInit()
{
   if(InpLots <= 0.0 ||
      ...
      InpGovernorAmberR < 0.0 ||
      InpGovernorRedR <= InpGovernorAmberR ||
      InpGovernorRecoverR < 0.0 ||
      InpGovernorRecoverR >= InpGovernorAmberR ||
      InpGovernorCooldownMin < 0 ||
      InpGovernorDailyFloorR < 0.0)
   {
   ...
   ResetGovernorDayIfNeeded();
   PrintFormat("%s initialized on %s; gap=%d SL=%d trail=%d act=%d DD amber=%.1fR red=%.1fR recover=%.1fR cooldown=%d floor=%.1fR",
               EA_TAG, _Symbol, InpGapPips, InpStopLossPips, InpTrailPips,
               InpTrailActivationPips, InpGovernorAmberR, InpGovernorRedR,
               InpGovernorRecoverR, InpGovernorCooldownMin, InpGovernorDailyFloorR);
   Reconcile();
